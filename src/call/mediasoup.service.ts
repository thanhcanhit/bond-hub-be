...
export class MediasoupService implements OnModuleInit, OnModuleDestroy {
  ...
  private async createWorkers() {
    ...
    for (let i = 0; i < numWorkers; i++) {
      try {
        ...
        this.logger.log(`Worker ${i + 1}/${numWorkers} created with pid ${worker.pid}`);
      } catch (error) {
  ...
  async createRouter(roomId: string): Promise<Router> {
    ...
    const router = await worker.createRouter({
      mediaCodecs: this.config.router.mediaCodecs as mediasoup.types.RtpCodecCapability[],
    });
   ...
   * @param direction 'send' or 'recv'
   */
  closeTransport(roomId: string, userId: string, direction: 'send' | 'recv'): void {
    const transportId = `${roomId}:${userId}:${direction}`;
    const transport = this.transports.get(transportId);
    if (transport) {
      transport.close();
