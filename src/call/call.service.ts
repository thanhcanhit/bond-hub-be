...
import { Call, CallParticipant, CallRoom, CallRoomParticipant } from './interfaces/call.interface';
...
export class CallService {
  ...
  async createCall(createCallDto: CreateCallDto): Promise<Call> {
    ...
    if ((receiverId && groupId) || (!receiverId && !groupId)) {
      throw new BadRequestException('Either receiverId or groupId must be provided, but not both');
    }
  ...
  async joinCall(joinCallDto: JoinCallDto): Promise<Call> {
    ...
    if (!call.groupId && call.initiatorId !== userId && !call.participants.some(p => p.userId === userId)) {
  ...
  async endCall(endCallDto: EndCallDto): Promise<Call> {
    ...
    if (call.status === CallStatus.ENDED || call.status === CallStatus.MISSED || call.status === CallStatus.REJECTED) {
   ...
   */
  async getCallHistory(userId: string, page = 1, limit = 20): Promise<{ calls: Call[]; total: number }> {
  ...
  getRoomParticipant(roomId: string, userId: string): CallRoomParticipant {
    ...
    if (!participant) {
      throw new NotFoundException(`Participant with ID ${userId} not found in room ${roomId}`);
    }
    return participant;
  }
   ...
   */
  setParticipantRtpCapabilities(roomId: string, userId: string, rtpCapabilities: any): void {
   ...
   */
  addProducerToParticipant(roomId: string, userId: string, producerId: string): void {
    const participant = this.getRoomParticipant(roomId, userId);
   ...
   */
  addConsumerToParticipant(roomId: string, userId: string, consumerId: string): void {
    const participant = this.getRoomParticipant(roomId, userId);
   ...
   */
  getOtherParticipants(roomId: string, excludeUserId: string): CallRoomParticipant[] {
    const room = this.getRoom(roomId);
